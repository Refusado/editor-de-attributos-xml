...
function displayXml($x){ // substitui os caracteres "<" e ">" para nao serem lidos como tags no navegador e possam ser imprimidos
  ...
  if (gettype($x) == "array"){
    ...
    echo "<div style='border: 1px solid black; width: fit-content; max-width: 80%; margin-bottom: 7px;'><code>$y</code></div>";
  }
};
...
for ($i = 0; $i < $groundsNo; $i++) {
  ...
  $padrao = "/P=\"$isdyn,$mass,$friction,$restit,$angle,$fixrotat,$lindamp,$angdamp\"/";
  $xml = preg_replace($padrao, "P=\"$isdyn,$mass,$friction,8,$angle,$fixrotat,$lindamp,$angdamp\"", $xml);
...
echo "<hr> Quantidade de pisos editados: $groundsNo <br>";